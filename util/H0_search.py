...
def read_file(fn):
    ...
    try:  # if no neutrinos, classy doesn't like being passed omega_ncdm
        if float(param_dict[OmegaNu]) == 0.0:
            del param_dict[OmegaNu]
    except Exception:
        pass
    return param_dict
...
def read_line(params,iCosm):
    ...
    for p,parName in enumerate(params.dtype.names):
        ...
        # omega_ncdm is typed as string (multi-species lists); compare via float
        if parName == OmegaNu:
            try:
                if float(str(params[parName][iCosm]).split(",")[0]) == 0.0:
                    continue
            except Exception:
                pass
