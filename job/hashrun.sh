...
# Sets: code_ref par2list nps nodes tmin overrides[] qsub_args[]
parse_args() {
    nps="" nodes="" tmin=""
    overrides=() qsub_args=()
    ...
    [[ -z ${tmin}  || ${tmin}  =~ ^[1-9][0-9]*$ ]] || die "-t/--time must be a positive integer number of minutes (got '${tmin}')"
}
...
submit() {
    ...
    local vlist="HASHRUN_SPEC=$spec" walltime=""
    local time_args=()
    if [[ -n $tmin ]]; then
